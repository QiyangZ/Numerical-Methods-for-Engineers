...
fai1 = zeros(1,n);
fai1 = forwardseuler(fai1,0,2,0.4);
%% Rückwärts-Euler_verfahren
fai2 = zeros(1,n);
fai2 = backwardseuler(fai2,0,2,0.4);
 %% Trapezregel
 fai3 = zeros(1,n);
fai3 = trapezrule(fai3,0,2,0.4);
...
for i = 2:n
    ...
    B = [-6,-6];
    C = [ft(t0(i)),ft(t0(i-1))];
    [LHS,RHS] = OST(0.0,0.4,M,B,C,faiost_zero(i-1));
...
for i = 2:n
    ...
    B = [-6,-6];
    C = [ft(t0(i)),ft(t0(i-1))];
    [LHS,RHS] = OST(0.5,0.4,M,B,C,faiost_half(i-1));
...
for i = 2:n
    ...
    B = [-6,-6];
    C = [ft(t0(i)),ft(t0(i-1))];
    [LHS,RHS] = OST(1.0,0.4,M,B,C,faiost_one(i-1));
...
title('Einschritt-θ-Verfahren mit timestep=0.4');
xlabel('t');
ylabel('Φ(t)');
...
title('Lösungen mit Zeitschritt = 0.4');
xlabel('t');
ylabel('Φ(t)');
